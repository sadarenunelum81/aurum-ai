...
export const EducationRecentPostsSection = ({ sectionConfig, themeMode }: { sectionConfig?: RecentPostsSectionConfig, themeMode?: 'light' | 'dark' }) => {
    ...
    const renderPostCard = (post: Article) => (
        <div key={post.id} className="group">
            <Link href={`/post/${post.id}`} className="block mb-4">
             ...
             <p className="text-xs font-semibold uppercase tracking-wider text-primary" style={{color: colors?.headerTextColor}}>{post.category || 'Article'}</p>
                        ...
                        {allPosts.length > visiblePosts.length && (
                            <div className="mt-16 text-center">
                                ...
                                >
                                    ...
                                    {sectionConfig.showMoreButtonText || 'Load More Courses'}
