...
export const EducationCategoriesSection = ({ config, themeMode }: { config?: TemplateConfig, themeMode?: 'light' | 'dark' }) => {
    ...
    const renderPostItem = (post: Article) => (
         <li key={post.id}>
             <Link href={`/post/${post.id}`} className="group flex items-center gap-2 text-sm hover:text-primary" style={{color: colors?.postTitleColor}}>
                <ArrowRight className="h-3 w-3 text-primary group-hover:translate-x-1 transition-transform" style={{color: colors?.postMetaColor}}/>
    ...
    const renderCategorySlot = (slot?: PopulatedCategorySlot, index?: number) => {
        ...
        return (
            <div key={index} className="border-t-4 pt-6" style={{borderColor: slot.color || 'hsl(var(--primary))'}}>
