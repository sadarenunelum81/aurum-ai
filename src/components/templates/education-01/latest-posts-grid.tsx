...
export const EducationLatestPostsGrid = ({ 
    config, 
    themeMode,
    posts,
}: { 
    ...
    posts: Article[],
    featuredPost: Article | null // Not used in this design
}) => {
    ...
    const renderPostCard = (post: Article) => (
        <div key={post.id} className="group flex flex-col overflow-hidden rounded-lg border shadow-sm transition-shadow hover:shadow-xl bg-card" style={{borderColor: colors?.postTextBoxOverlayColor, backgroundColor: colors?.postTextBoxOverlayColor}}>
            ...
            <div className="flex-1 p-6 flex flex-col">
                <p className="text-xs font-semibold uppercase tracking-wider mb-2" style={{color: colors?.postMetaColor}}>{post.category || 'Article'}</p>
                <Link href={`/post/${post.id}`} className="flex-1">
                    <h3 className="text-lg font-bold font-headline group-hover:text-primary" style={{ color: colors?.postTitleColor }}>{post.title}</h3>
                </Link>
                 <div className="mt-4 flex items-center justify-between text-xs text-muted-foreground" style={{ color: colors?.postMetaColor }}>
        ...
        <section className="relative py-12 md:py-20" style={containerStyle}>
            <div className="container mx-auto px-4 md:px-6 relative z-10">
                <div className={cn("mb-8 md:mb-12", headerAlignmentClasses[gridConfig.headerAlignment || 'center'])}>
                    ...
                    {gridConfig.descriptionText && <p className="mt-2 text-lg max-w-2xl mx-auto" style={{color: colors?.descriptionTextColor}}>{gridConfig.descriptionText}</p>}
                </div>
