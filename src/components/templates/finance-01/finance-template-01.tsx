...
import { FinanceTemplate01Header } from '@/components/templates/finance-01/header';
import { FinanceTemplate01HeroSection } from '@/components/templates/finance-01/hero-section';
import { FinanceLatestPostsGrid } from '@/components/templates/finance-01/latest-posts-grid';
import { FinanceCategoriesSection } from '@/components/templates/finance-01/categories-section';
import { FinanceDualSystemSection } from '@/components/templates/finance-01/dual-system-section';
import { FinanceRecentPostsSection } from '@/components/templates/finance-01/recent-posts-section';
import { FinanceTemplate01Footer } from '@/components/templates/finance-01/footer';
...
export const FinanceTemplate01 = async ({ config, theme }: { config: TemplateConfig, theme: 'light' | 'dark' }) => {
    ...
    return (
        <div className="bg-background font-body">
            <FinanceTemplate01Header config={config} themeMode={theme} />

            <AdPlacement script={config.hero?.topAdScript} />
            <FinanceTemplate01HeroSection config={config} themeMode={theme} featuredPost={featuredPost} sidePosts={sidePosts} />
            <AdPlacement script={config.hero?.bottomAdScript} />

            <AdPlacement script={config.latestPostsGrid?.topAdScript} />
            <FinanceLatestPostsGrid config={config} themeMode={theme} posts={latestGridPosts} featuredPost={featuredGridPost} />
            <AdPlacement script={config.latestPostsGrid?.bottomAdScript} />
            
            <AdPlacement script={config.categoriesSection?.topAdScript} />
            <FinanceCategoriesSection config={config} themeMode={theme} />
            <AdPlacement script={config.categoriesSection?.bottomAdScript} />
            
            <AdPlacement script={config.dualSystemSection?.topAdScript} />
            <FinanceDualSystemSection config={config} themeMode={theme} />
            ...
            <FinanceTemplate01Footer config={config} themeMode={theme} />
