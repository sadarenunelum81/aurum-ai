...
import { Loader2, Upload, Copy, Settings, Plus, Trash2, ArrowUp, ArrowDown, Image as ImageIcon, Pilcrow } from 'lucide-react';
import {
    uploadImageAction,
...
function htmlToBlocks(html: string): ContentBlock[] {
    if (!html) return [{ id: `p${Date.now()}`, type: 'paragraph', content: '' }];
    ...
    tempDiv.childNodes.forEach(node => {
        if (node.nodeName === 'P') {
            blocks.push({
                ...
                type: 'paragraph',
                content: (node as HTMLParagraphElement).innerText || ''
            });
        } else if (node.nodeName === 'DIV' && (node as HTMLElement).querySelector('img')) {
             const img = (node as HTMLElement).querySelector('img');
             if(img?.src) {
                blocks.push({
                    id: `i${idCounter++}-${Date.now()}`,
                    type: 'image',
                    content: img.src
                });
             }
    ...
    if (blocks.length === 0) {
        return [{ id: `p${Date.now()}`, type: 'paragraph', content: tempDiv.innerText || '' }];
    }
        ...
        <div className="flex-1 p-4 md:p-6 lg:p-8">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Refine your article's details and content using the block editor.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-8">
                    <div className="space-y-2">
